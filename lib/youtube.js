...
// concurrentViewers vive en Videos.liveStreamingDetails, no en el recurso
// LiveBroadcast (que solo trae snippet/status) - el id del broadcast y el
// del video son el mismo id.
async function getViewerCount() {
  const tokenData = await ensureToken();
  const broadcast = await getActiveBroadcast();
  if (!broadcast) return null;
  const res = await fetch(
    `https://www.googleapis.com/youtube/v3/videos?part=liveStreamingDetails&id=${broadcast.id}`,
    { headers: { Authorization: `Bearer ${tokenData.access_token}` } }
  );
